...
def main(config: DictConfig) -> None:
    ...
    object_dir = os.path.join(custom_output_path, "object")
    os.makedirs(object_dir, exist_ok=True)

    curr_folder_count = len([name for name in os.listdir(object_dir) if os.path.isdir(os.path.join(object_dir, name))])
    test_folder_name = f"object_test_{curr_folder_count + 1}"
    test_folder = os.path.join(object_dir, test_folder_name)
    ...
    for scene_i, scene_id in enumerate(scene_ids):
        ...
        for task_id in range(len(object_nav_task.task_dict)):
            ...
            object_categories = parse_object_goal_instruction(object_nav_task.instruction, "zero-shot")
            ...
            # map_image = cv2.imread("sem_map.png")
