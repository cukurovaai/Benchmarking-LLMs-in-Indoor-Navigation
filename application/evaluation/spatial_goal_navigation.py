...
def main(config: DictConfig) -> None:
    ...
    spatial_dir = os.path.join(custom_output_path, "spatial") 
    os.makedirs(spatial_dir, exist_ok=True)

    curr_folder_count = len([name for name in os.listdir(spatial_dir) if os.path.isdir(os.path.join(spatial_dir, name))])
    test_folder_name = f"spatial_test_{curr_folder_count + 1}"
    test_folder = os.path.join(spatial_dir, test_folder_name)
    os.makedirs(test_folder, exist_ok=True)
    
    trajectory_folder = os.path.join(test_folder, "trajectory")
    ...
    for scene_i, scene_id in enumerate(scene_ids):
        ...
        # map_image = cv2.imread("sem_map2.png")
        map_image = get_topdown_map_from_sim(robot.sim)
        cv2.imwrite(os.path.join(test_folder, f"map.png"), map_image)

        for task_id in range(len(spatial_nav_task.task_dict)):
            spatial_nav_task.setup_task(task_id)
            task_arr = parse_spatial_goal_instruction(spatial_nav_task.instruction, "zero-shot")
            ...
            for action_i, action in enumerate(recorded_actions_list):
                spatial_nav_task.test_step(robot.sim, action, vis=config.nav.vis, task="empty", task_id=task_id)

                obs = robot.sim.get_sensor_observations(0)
                save_obs(test_folder, sim_setting, obs, action_i, None)
            ...
            for count, state in enumerate(agent_states):
                ...
                print(f"saving trajectory output {count} into {trajectory_folder}")
                agent_image = draw_agent(path_image.copy(), real_states[-1], quaternion_to_yaw(state.rotation))
                cv2.imwrite(os.path.join(trajectory_folder, f"path_{count}.png"), agent_image)

            save_states(test_folder, agent_states)
            save_tasks(task_arr, os.path.join(test_folder, 'tasks.txt'))
            break
