...
def main(config: DictConfig) -> None:
    ...
    for scene_i, scene_id in enumerate(scene_ids):
        ...
        if scene_i == 2:
            result_code = parse_spatial_goal_instruction(spatial_nav_task.instruction, "reasoning")
        else:
            result_code = parse_spatial_goal_instruction(spatial_nav_task.instruction, "spatial")
        
        result_codes.append(spatial_nav_task.instruction)
        result_codes.append(result_code)
        print(f"instruction {scene_i}: {spatial_nav_task.instruction}")
        print(f"result code:\n {result_code}")
        ...
        save_tasks(result_codes, os.path.join(test_dir, 'codes.txt'))
