...
def dic2txt(dictionary, file_path):
    with open(file_path, 'w') as file:
        for key, value in dictionary.items():
            file.write(f"{key}: {value[0]}, {value[1]}\n")

def generate_scene_data(save_dir: Union[Path, str], config: DictConfig, scene_path: Path, poses: np.ndarray, i) -> None:
    """
    ...
    obj2cls_path = os.path.join(save_dir, "obj2cls_dict.txt")
    dic2txt(obj2cls, obj2cls_path)
...
def main(config: DictConfig) -> None:
    ...
    for data_dir_i, data_dir in enumerate(pbar):
        ...
        generate_scene_data(data_dir, config.data_cfg, scene_path, poses, data_dir_i)
